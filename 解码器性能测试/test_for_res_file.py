...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        self.load_csv.clicked.connect(self.load_csv_encoding)
        self.play_wav.clicked.connect(self.play_wav_file)
    ...
    def play_wav_file(self):
        filename = self.load_wav.toPlainText()
        print(filename)
        if filename == '':
            msgBox = QMessageBox()
    ...
    def load_csv_encoding(self):
        csv_content = []
        f = open('content.csv', 'r', encoding='utf-8')
        rdr = csv.reader(f)
        i = 0
        for line in rdr:
            temp = ''
    ...
    def decode(self):
        ...
        plt.plot(read_signal[30000:120000])
        plt.grid()
        ...
        plt.plot(filtedData[30000:120000])
        plt.grid()
        ...
        while 1:
            ...
            for i in range(current_length - len(preamble_y)):
                ...
                if once_check + 57600 == i and once_check != 0:
                    print('breakbreak')
                    break
            ...
            writing = str(decode_payload_length)+decode_payload
            wr = csv.writer(f)
            wr.writerow(writing)
